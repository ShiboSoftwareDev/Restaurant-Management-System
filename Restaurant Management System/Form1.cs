    ...
    public partial class Form1 : Form
    {
        ...
        private void SetupItemsPanel()
        {
            ...
            addItemBtn.Click += (s, e) => {
                using (SqlConnection conn = new SqlConnection(connectionString))
                {
                    ...
                    {
                        conn.Open();
                        var cmd = new SqlCommand("INSERT INTO MenuItems (Name, Price, Category) VALUES (@name, @price, @category)", conn);
        ...
        private void LoadMenuItems()
        {
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                ...
                {
                    conn.Open();
                    var adapter = new SqlDataAdapter("SELECT Name, Price, Category FROM MenuItems", conn);
                    var table = new DataTable();
                    adapter.Fill(table);
