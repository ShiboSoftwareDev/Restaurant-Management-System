    ...
    public partial class MainForm : Form
    {
        private void SetupClientsPanel()
        {
            ...
            addClientBtn.Click += (s, e) => {
                ...
                Restaurant_Management_System.DAL.ClientsDAL.AddClient(connectionString, name);
                LoadClients(clientsGrid);
            };

            var deleteClientBtn = new Button { Text = "Delete Selected", Height = 44, Width = 160, BackColor = Color.FromArgb(220, 53, 69), ForeColor = Color.White, FlatStyle = FlatStyle.Flat, Font = new Font("Segoe UI", 12, FontStyle.Bold), Cursor = Cursors.Hand };
            deleteClientBtn.FlatAppearance.BorderSize = 0;
            deleteClientBtn.Click += (s, e) => {
                if (clientsGrid.SelectedRows.Count > 0)
                {
                    var row = clientsGrid.SelectedRows[0];
                    if (row.DataBoundItem is DataRowView drv)
                    {
                        int clientId = Convert.ToInt32(drv["ClientId"]);
                        var confirm = MessageBox.Show($"Delete client with ID {clientId}?", "Confirm Delete", MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
                        if (confirm == DialogResult.Yes)
                        {
                            Restaurant_Management_System.DAL.ClientsDAL.DeleteClient(connectionString, clientId);
                            LoadClients(clientsGrid);
                        }
                    }
                }
            ...
            addClientPanel.Controls.Add(deleteClientBtn);
